...
# Separate for conditions
Saccades_search <- subset(Saccades_forage, SearchOrCount == "Search")
Saccades_count <- subset(Saccades_forage, SearchOrCount == "Count")


plot(density(Saccades_search$Amplitude))
plot(density(Saccades_count$Amplitude))

# There are more long saccades in the search condition because you may go from one corner to a completely different corner.
# A lot more short saccades in the count condition

ggplot()+
  geom_density(aes(Amplitude, fill = SearchOrCount), alpha = 0.5, data = Saccades_forage)+
  theme_light()
```
...
mGaus <- glmer(
  ...
  family = gaussian(link = "identity"), #gaussian? means assuming normal distribution. identity link? means
  data = Saccades_forage)

summary(mGaus)
# Okay, 
# the unit is 'degrees of visual angle'
# the are not super normal, however, because they are very skewed to
```
...
```{r}
Saccades_forage <- Saccades_forage %>% filter(Amplitude > 0)
...
# what does 'log normal' mean? (family = gaussian(link = "log"))
# it means applying a log transformation, and "moving the mode towards the center and cutting the tail so everything gets shifted a little bit to the right and it looks more normal."
```
...
Numerically compare the model predictions to the actual data (in absolute values) in order to get an idea of how well the model predicts the data:

```{r}
summary(abs(pm1- Saccades_forage$Amplitude))
summary(abs(pm2- Saccades_forage$Amplitude))
...
```{r}
# Separate for papers
Fixations_forage <- subset(Fixations, Task == "Visual foraging")
...
```{r}
# Effect of ostensiveness
ggplot(data = Fixations_soceng)+
  geom_density(aes(PupilSize, fill = Ostensiveness), alpha = 0.5)+
  theme_light()

mean(Fixations_soceng$PupilSize[Fixations_soceng$Ostensiveness == "Ostensive"])
mean(Fixations_soceng$PupilSize[Fixations_soceng$Ostensiveness == "Non-ostensive"])

# Effect of direction
ggplot(data = Fixations_soceng)+
  geom_density(aes(PupilSize, fill = Direction), alpha = 0.5)+
  theme_light()

mean(Fixations_soceng$PupilSize[Fixations_soceng$Direction == "Direct"])
mean(Fixations_soceng$PupilSize[Fixations_soceng$Direction == "Indirect"])
```
...
```{r}
mPupil <- glmer(PupilSize ~ Ostensiveness * Direction + (1 | ParticipantID), data = Fixations_soceng)
...
```{r}
ggplot(data = Fixations_soceng)+
  geom_density(aes(Duration, fill = Ostensiveness, color = Direction), alpha = 0.5)+
  theme_light()
